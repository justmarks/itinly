...
- **Places to go** — per-trip list of points-of-interest (museums, viewpoints, neighbourhoods) that aren't tied to a date or time. Each place takes a name, address, and/or URL; appears as an orange pin on the Map view; groupable by city. Future "schedule this" actions can promote a place into a real segment.
...
Current coverage: **1052 tests** across 66 test suites.
...
| `SENTRY_DSN` | server | Sentry error-reporting DSN. Unset in dev/CI disables Sentry entirely. Failed scheduled email scans, parse-pipeline exceptions, and other server errors are routed here via `server/src/services/monitoring.ts`. |
...
| `DEBUG_EMAIL_SCHEDULE` | server | Set to `1` to enable scheduled email-scan lifecycle logs in Railway: cron tick fan-out, schedules picked up, per-schedule start/finish, cadence advance, and the run's final status / next-run timestamp. Failures already alert Sentry (and `console.error`) regardless, so this knob is only for tracking the happy path (success / "nothing new") without flooding Railway logs in steady state. |
...
| `NEXT_PUBLIC_SENTRY_DSN` | apps/web | Sentry browser DSN. Must be `NEXT_PUBLIC_` to be embedded in the static bundle. Unset disables Sentry on the client. |
...
- [x] **Calendar sync on shared trips** — recipients of a shared-edit trip can push the same trip to their own Google / Outlook calendar independently of the owner. New `trip_user_calendar_syncs` table (`server/drizzle/0007_per_user_calendar_sync.sql`) holds `(tripId, userId, calendarId, segmentEventMap)` per user, with the migration backfilling the owner's existing sync state into the new shape so no re-sync is required. Calendar-sync routes read/write keyed by `req.userId` instead of mutating the trip row, so two users syncing the same trip can't clobber each other's event ids. UI gate flipped from `isOwner` to `canEdit`, with the per-user state surfaced consistently across desktop trip-actions menu and mobile calendar sheet
...
- [x] **Places to go** — per-trip list of points-of-interest (shops, museums, viewpoints, neighbourhoods) that aren't tied to a date or time and aren't a todo. Each place takes a name, address, and/or URL; renders as an orange pin on both the desktop Map view and the mobile full-screen map alongside the four itinerary categories (transport / lodging / activity / dining); groupable by city in the dedicated Places tab. Persisted to a new `places` Postgres table with RLS gating per owner; future "schedule this" actions can promote a place into a real segment
